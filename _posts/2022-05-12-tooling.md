...
Not every computer is created equal. Neither is every OS. I began my IT path on a Macbook Air from 2014 with 8 GB of RAM. While this is enormous compared to the [Appolo Guidance Computer](https://en.wikipedia.org/wiki/Apollo_Guidance_Computer), it is not enough for modern-day software development. Tools like Visual Studio and most of Jetbrains products will make your machine sweat. Faced with the conundrum of limited resources, the IT student has two options: (a) beef up his/her hardware or (b) outsource resource-intensive tasks to stronger machines. I suggest to do both.

My experience is that the limiting factors are RAM and I/O speed to disk. Most people I have met program on laptops, so let's focus on this scenario. A few exceptions aside - like the [Framework](https://frame.work/de/en) products -  laptops are not particularly modular, meaning extensible. If you make a mistake on your purchase, you will have a tough time correcting the shot. This makes it all the more important to identify the purchasing criteria precisely.
...
Talking about virtual machines. Back in 2019, the words "virtual machine" (vm) brought images of nebulosity to my mind. Like the word "cloud" and the like. Three years later I have fallen in love with this technology, perhaps at a time when a lot of developers are moving on from it. Indeed, the new hot thing on the block - at least on Windows - is [WSL2](https://docs.microsoft.com/en-us/windows/wsl/install). I will not go into a detailed comparison of the two systems. 

My view is that it boils down to worflow preference. Some say booting a vm is slow. I personally like the fact that my vm takes 60 seconds to be ready. At its core, virtualisation is about apportioning the physical resources of your machine and allocating some of them to a segregated construct in which to run processes. It is therefore about isolation. On my set-up, a vm takes a minute to stabilise and - while a machine can perform the same functionality faster (e.g. with WSL2) - my brain needs this minute to transition from what happens locally to what happens remotely. This slow boot time gives me the opportunity to get prepared to work on a new project which will likely live on a separate vm.

> Machines are ever quicker, but my brain has a human clockspeed.

Allow me to close this philosophical parenthese. 

On Windows, I recommend to use Hyper-V for virtualisation. As far as I know, it offers the best performance on that platform. On Linux, I suggest to explore KVM and Proxmox. For users on Windows, I have written a [powershell script](https://gist.github.com/saxomoose/2ea31c798ee489f58acc9940a628720b) to automate the vm creation process. Do note that you may want to create a [NAT virtual switch](https://docs.microsoft.com/en-us/virtualization/hyper-v-on-windows/user-guide/setup-nat-network) beforehand. The reason being that the Hyper-V Default Switch hands out ip addresses via DHCP. It is not straightforward to tweak the settings to assign a fixed ip to newly created vms. This is why I prefer to use the NAT switch, despite a small network performance hit.